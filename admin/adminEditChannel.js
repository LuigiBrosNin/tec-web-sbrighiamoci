...
function populate(channel_json) {
    ...
    for (let index in channel_json.mod_list) {
        document.getElementById("modList").innerHTML += `<div class="modCard" id="mod_${index}"><p>${channel_json.mod_list[index]}</p><button class="modBtn" type="button" onclick="event.preventDefalult; removeMod('${channel_json.mod_list[index]}', ${index})">Remove</button></div>`;
    }
